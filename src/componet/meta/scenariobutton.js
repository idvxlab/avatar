...
export default class ScenarioButton extends Component {
    constructor(props){
        super(props)
        this.state={peeps:this.props.peeps,x:this.props.x,y:this.props.y,style:this.props.style,l:this.props.l,E:this.props.E}        
    }
...
render(){
    if(this.state.peeps){
        if(this.state.style===1){
            return(<div onClick={()=>{this.props.transscenario2(this.props.id)}}><div style={{border: '3px solid #4D59BF',/* Rectangle 41 */
        position: 'absolute',width: this.state.l-6+'px',height:this.state.l-6+'px',left: this.state.x+'px',top: this.state.y+'px',background:'#FFFFFF',boxShadow: '0px 4px 9px rgba(0, 0, 0, 0.04)',}}>
            <div style={{position:'absolute',left:this.state.l*0.05-3,top:this.state.l*0.05-3}}><img src={this.state.peeps}  width={this.state.l*0.9} height={this.state.l*0.9}/></div></div></div>)
        }
        ...
        position: 'absolute',width: this.state.l+'px',height:this.state.l+'px',left: this.state.x+'px',top: this.state.y+'px',background:'#FFFFFF',boxShadow: '0px 4px 9px rgba(0, 0, 0, 0.04)',}}>
            <div style={{position:'absolute',left:this.state.l*0.05,top:this.state.l*0.05}}><img src={this.state.peeps}  width={this.state.l*0.9} height={this.state.l*0.9}/></div></div></div>)
